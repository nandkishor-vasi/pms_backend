...
public class ProjectRepo {
    ...
    public List<ProjectModel> findByMemberId(Long memberId) {
        String sql = "SELECT p.* FROM projects p " +
                "LEFT JOIN project_members pm ON p.id = pm.project_id " +
                "WHERE p.created_by = ? OR pm.user_id = ?";

        List<ProjectModel> projects = jdbcTemplate.query(sql, new Object[]{memberId, memberId}, new RowMapper<ProjectModel>() {
            @Override
            public ProjectModel mapRow(ResultSet rs, int rowNum) throws SQLException {
                ProjectModel project = new ProjectModel();
                project.setId(rs.getLong("id"));
                ...
                // Note: createdBy and members would need separate queries to fully populate
                // as they are complex relationships
        ...
        if (projects.isEmpty()) {
            System.out.println("No projects found for user ID: " + memberId);
        }
